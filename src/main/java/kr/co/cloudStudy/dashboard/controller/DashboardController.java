...
public class DashboardController implements DashboardControllerDocs {
	...
	public ResponseEntity<ApiResponseDTO<DashboardResponseDTO>> getDashboard(
		...
		DashboardResponseDTO response = dashboardService.getDashboard(extractEmployeeNumber(userDetails));

		return ResponseEntity.ok(ApiResponseDTO.success("대시보드 조회 성공", response));
	}
	...
	public ResponseEntity<ApiResponseDTO<DashboardTodayAttendanceResponseDTO>> getTodayAttendance(
			@AuthenticationPrincipal CustomUserDetails userDetails) {
		DashboardTodayAttendanceResponseDTO response = dashboardService
				.getTodayAttendance(extractEmployeeNumber(userDetails));

		return ResponseEntity.ok(ApiResponseDTO.success("오늘 근태 상태 조회 성공", response));
	}
	...
	public ResponseEntity<ApiResponseDTO<DashboardRecentActivityResponseDTO>> getRecentActivities(
		...
		return ResponseEntity.ok(ApiResponseDTO.success("최근 활동 조회 성공", response));
	}
	...
	public ResponseEntity<ApiResponseDTO<DashboardTodayAttendanceResponseDTO>> checkIn(
			@AuthenticationPrincipal CustomUserDetails userDetails) {
		DashboardTodayAttendanceResponseDTO response = dashboardService.checkIn(extractEmployeeNumber(userDetails));

		return ResponseEntity.ok(ApiResponseDTO.success("출근 처리 성공", response));
	}
	...
	public ResponseEntity<ApiResponseDTO<DashboardTodayAttendanceResponseDTO>> checkOut(
		...
		return ResponseEntity.ok(ApiResponseDTO.success("퇴근 처리 성공", response));
	}
